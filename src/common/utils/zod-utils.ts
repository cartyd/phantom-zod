...
export function makeOptional<T extends z.ZodTypeAny>(schema: T) {
  return z.union([schema, z.undefined(), z.null().transform(() => undefined)]);
}
 ...
 * ```
 */
export function makeOptionalSimple<T extends z.ZodTypeAny>(
  schema: T,
  errorMessage?: string,
) {
  const unionOptions = errorMessage ? { message: errorMessage } : undefined;
  return z.union([schema, z.undefined()], unionOptions);
}
...
export function addArrayConstraints<T extends z.ZodTypeAny>(
  ...
  if (constraints.min !== undefined) {
    result = result.refine(
      ...
      {
        message:
          errorMessages.tooSmall?.(constraints.min) ||
          `Must have at least ${constraints.min} items`,
      },
    );
  }
  ...
  if (constraints.max !== undefined) {
    result = result.refine(
      ...
      {
        message:
          errorMessages.tooBig?.(constraints.max) ||
          `Must have at most ${constraints.max} items`,
      },
    );
  }
